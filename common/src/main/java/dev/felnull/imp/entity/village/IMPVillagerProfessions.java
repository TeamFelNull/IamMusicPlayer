...
// OERegistryUtil.createVillagerProfession(new ResourceLocation(IamMusicPlayer.MODID, name), poiType.get(), ImmutableSet.of(), ImmutableSet.of(), soundEvent)
            return new VillagerProfession(new ResourceLocation(IamMusicPlayer.MODID, name).toString(), (holder) -> {
                return holder.is(ResourceKey.createRegistryKey(poiType.getRegistryId()));
            }, (holder) -> {
