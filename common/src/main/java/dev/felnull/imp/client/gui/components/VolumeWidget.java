...
public class VolumeWidget extends AbstractWidget implements IIMPSmartRender {
    ...
    public void renderButton(PoseStack poseStack, int i, int j, float f) {
        ...
        OERenderUtils.drawTexture(MusicManagerMonitor.WIDGETS_TEXTURE, poseStack, getX(), getY(), z, 156, imute ? 4 : 4 + lfs, 8);
        if (imute)
            OERenderUtils.drawTexture(MusicManagerMonitor.WIDGETS_TEXTURE, poseStack, getX() + 3, getY(), 12 + z, 156, 8, 8);
        drawSmartText(poseStack, Component.literal(String.valueOf(vol)), getX() + 5 + lfs, getY() + 0.5f, isHoveredOrFocused() ? 0XFF007F06 : 0XFF115D0E);
    }
