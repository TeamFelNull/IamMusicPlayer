...
public class YoutubePlayListMusicsFixedListWidget extends IMPBaseFixedListWidget<ImportYoutubePlayListMMMonitor.YoutubePlayListEntry> {
    ...
    @Override
    protected void renderOneButton(PoseStack poseStack, ImportYoutubePlayListMMMonitor.YoutubePlayListEntry item, int lnum, int bnum, int bX, int bY, int mx, int my, float parTick, boolean selected) {
        ...
        drawSmartFixedWidthText(poseStack, Component.literal(item.name()), bX + sx, bY + 2, getIndividualWidth() - sx - 2);
        drawSmartFixedWidthText(poseStack, Component.literal(item.artist()), bX + sx, bY + 12, getIndividualWidth() - sx - 2);
    }
}
