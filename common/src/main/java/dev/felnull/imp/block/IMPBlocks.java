...
public class IMPBlocks {
    ...
    public static final RegistrySupplier<Block> BOOMBOX = register("boombox", () -> new BoomboxBlock(BlockBehaviour.Properties.of(Material.METAL).sound(SoundType.LANTERN).strength(3.0F)), n -> new BoomboxItem(n, new Item.Properties().arch$tab(IMPCreativeModeTab.MOD_TAB).stacksTo(1)));

    private static RegistrySupplier<Block> register(String name, Supplier<Block> block) {
        return register(name, block, n -> new BlockItem(n, new Item.Properties().arch$tab(IMPCreativeModeTab.MOD_TAB)));
    }
