...
public class MusicClientInstructionMessage implements IPacketMessage {
    ...
    public MusicClientInstructionMessage(Type type, UUID uuid, long time, MusicPlayInfo info) {
        this(type, uuid, time, MusicSource.EMPTY, info);
    }

    public MusicClientInstructionMessage(Type type, UUID uuid, long time, MusicSource location) {
        this(type, uuid, time, location, MusicPlayInfo.EMPTY);
    }

    public MusicClientInstructionMessage(Type type, UUID uuid, long time, MusicSource location, MusicPlayInfo info) {
        this.type = type;
        this.uuid = uuid;
        this.time = time;
    ...
    public void decode(FriendlyByteBuf byteBuf) {
        ...
        this.location = new MusicSource(byteBuf.readAnySizeNbt());
