...
public class CassetteTapeItemRenderer implements ICustomBEWLRenderer {
    ...
    public void renderByItem(ItemStack itemStack, ItemTransforms.TransformType transformType, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int i1) {
        ...
        renderTapeCore(poseStack, par, par, ivb, 1.75d, -0.01f, 2.5d, i, i1);
        renderTapeCore(poseStack, par, par, ivb, 7.25d, -0.01f, 2.5d, i, i1);
    }

    public static void renderTapeCore(PoseStack poseStack, float par, float roll, VertexConsumer ivb, double x, double y, double z, int i, int i1) {
        ...
        roll *= 4f;
        float rollPar = roll + 0.75f;
        for (int j = 1; j <= Math.ceil(rollPar); j++) {
            poseStack.pushPose();
            IKSGRenderUtil.matrixTranslatef16Divisions(poseStack, 0.4, 0, 0.4);
            IKSGRenderUtil.matrixScalf(poseStack, Math.min(rollPar, j));
            IKSGRenderUtil.matrixTranslatef16Divisions(poseStack, -0.4, 0, -0.4);
