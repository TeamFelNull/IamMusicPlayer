...
public class SoundPlayMessage {
	...
	public SoundPlayMessage(int dimID, BlockPos postion, int state) {
		this(dimID, postion, state, "null");
	}

	public SoundPlayMessage(int dimID, BlockPos postion, int state, String st) {
		this.dim = dimID;
		this.pos = postion;
		this.state = state;
		this.string = st;
	...
	public static SoundPlayMessage decodeMessege(PacketBuffer buffer) {

		return new SoundPlayMessage(buffer.readInt(),
				new BlockPos(buffer.readInt(), buffer.readInt(), buffer.readInt()), buffer.readInt(),
				buffer.readString(32767));
	}

	public static void encodeMessege(SoundPlayMessage messegeIn, PacketBuffer buffer) {
