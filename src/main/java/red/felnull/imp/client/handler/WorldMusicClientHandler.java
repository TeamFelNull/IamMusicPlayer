...
public class WorldMusicClientHandler {
    ...
    public static void onServerResponse(ResponseEvent.Server e) {
        if (e.getLocation().equals(IMPWorldData.WORLDMUSICFILEDATAINFO)) {
            if (e.getId() == 0) {
                MusicDownloader.instance().WAITINFOUUID.put(UUID.fromString(e.getMessage()), new WorldMusicFileDataInfo(e.getData()));
            } else if (e.getId() == 1) {
                MusicDownloader.instance().WAITINFOUUID.put(UUID.fromString(e.getMessage()), new WorldMusicFileDataInfo(true));
            }
        }
    }
}
