...
public class YoutubeSearchResultScrollListButton extends ScrollListButton {
    ...
    public void renderOneList(MatrixStack matrix, int x, int y, int num, int upOver, int downOver) {
        ...
        if (upOver < 13 && downOver <= 37) {
            IKSGRenderUtil.drawHorizontalMovementString(matrix, fontrenderer, pl.getInfo().title, "imp.msdfc." + pl.hashCode() + " file.hashCode()", 30, x + 41, y + 3, 133, 30);
        }
        ...
        if (screen.musicPlayThread != null && screen.musicPlayThread.isMusicPlayLoading() && screen.musicPlayThread.getMusicPlayLodingSrc() != null && screen.musicPlayThread.getMusicPlayLodingSrc().equals(pl.getIdentifier())) {
            ...
            if (upOver < 38 && downOver <= 12) {
                IKSGRenderUtil.drawString(fontrenderer, matrix, IKSGStyles.withStyle(new TranslationTextComponent("msd.musicloading")), x + 50, y + 29, 0);
            }
        } else {
