...
public class ClientSendSoundFileMessage {
    ...
    public ClientSendSoundFileMessage(String name, boolean stop) {
        this(new byte[1], false, 0, name, ClientSoundFileSender.SendFolderType.MAIN, stop);
    }

    public ClientSendSoundFileMessage(byte[] byetes, String name) {
        this(byetes, false, 0, name, ClientSoundFileSender.SendFolderType.MAIN);
    }

    public ClientSendSoundFileMessage(byte[] byetes, boolean frist, int length, String name, ClientSoundFileSender.SendFolderType type) {
    ...
    public ClientSendSoundFileMessage(byte[] byetes, boolean frist, int length, String name,
                                      ClientSoundFileSender.SendFolderType type, boolean stop) {
    ...
    public static ClientSendSoundFileMessage decodeMessege(PacketBuffer buffer) {
        ...
        return new ClientSendSoundFileMessage(buffer.readByteArray(), buffer.readBoolean(), buffer.readInt(),
                buffer.readString(32767), ClientSoundFileSender.SendFolderType.valueOf(buffer.readString(32767).toUpperCase()),
                buffer.readBoolean());
    }
