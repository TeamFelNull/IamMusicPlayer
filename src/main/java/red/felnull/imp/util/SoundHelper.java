...
public class SoundHelper {
    ...
    public static float getVolumeFromSoundPos(SoundPos pos, float motovol) {
        ...
        double kyori = pos.distance(mc.player.getPosX(), mc.player.getPosY() + 1, mc.player.getPosZ());
    ...
    public static long getSoundLength(PlayData pd, MinecraftServer ms) {
        ...
        if (pd.type == PlayData.PlayDatasTypes.FILE) {
            try {
        ...
        } else if (pd.type == PlayData.PlayDatasTypes.WORLD) {
            try {
        ...
        } else if (pd.type == PlayData.PlayDatasTypes.URL_STREAM) {
            return -1;
        }
