...
public class PlayMusicCreateRequestMessageHandler {
    public static void reversiveMessage(PlayMusicCreateRequestMessage message, Supplier<NetworkEvent.Context> ctx) {
        PlayMusicManeger.instance().createPlayMusic(ctx.get().getSender(), message.name, message.imageUUID, message.imageW, message.imageH, message.musicUUID, message.artist, message.album, message.year, message.genre, message.bitrate, message.lethsec);
    }
}
