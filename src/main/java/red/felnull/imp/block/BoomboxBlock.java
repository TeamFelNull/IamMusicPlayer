...
import red.felnull.imp.item.BoomBoxTileEntityStack;
...
public class BoomboxBlock extends Block implements IWaterLoggable {
    ...
    public BoomboxBlock(Properties properties) {
        super(properties.func_235838_a_((sr) -> {
            return sr.getBlockState().get(ON) ? 5 : 0;
        }));
        this.setDefaultState(
                this.stateContainer.getBaseState().with(FACING, Direction.NORTH).with(OPEN, Boolean.valueOf(false))
                        .with(ON, Boolean.valueOf(false)).with(VOLUME, 24).with(WATERLOGGED, Boolean.valueOf(false))
    ...
    public ActionResultType onBlockActivated(BlockState stateIn, World worldIn, BlockPos pos,
        ...
        if (brtr.getFace() == stateIn.get(FACING).rotateY()) {
            ...
            if (player.isCrouching()) {
                stateIn = stateIn.func_235896_a_(OPEN);
            ...
            if (!player.isCrouching() && !stateIn.get(OPEN) && SoundHelper.canPlay(tileentity.getCassette())
                ...
                stateIn = stateIn.func_235896_a_(ON);
                worldIn.setBlockState(pos, stateIn, 2);
                return ActionResultType.SUCCESS;
        ...
        if (!cassette.isEmpty()) {
            ...
            if (tossed) {
                ...
                if (direction == Direction.NORTH) {
                    ...
                    dropItem.setMotion(new Vector3d(0.3f, 0, 0));
                    ...
                    dropItem.setMotion(new Vector3d(0, 0, 0.3f));
    ...
    public boolean isValidPosition(BlockState state, IWorldReader worldIn, BlockPos pos) {
        ...
        if (state.get(WALL)) {
            return hasEnoughSolidSide(worldIn, pos.offset(state.get(FACING).rotateY().rotateY().rotateY()),
                    state.get(FACING));
        }
